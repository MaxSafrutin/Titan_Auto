function taskList(filters) {
  return sortNewest(listRecords('TASKS').filter(function(x){return (!(filters||{}).vehicle_id||String(x.vehicle_id)===String(filters.vehicle_id))&&(!(filters||{}).lead_id||String(x.lead_id)===String(filters.lead_id));}));
}
